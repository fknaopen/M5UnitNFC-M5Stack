...
TEST(APDU, MakeCommandCases)
{
    ...
    EXPECT_EQ(c3.size(), 4u + 1u + sizeof(data3));
    if (c3.size() >= 5u) {
    ...
    EXPECT_EQ(c3e.size(), 4u + 3u + data_long.size());
    if (c3e.size() >= 7u) {
    ...
    EXPECT_EQ(c4.size(), 4u + 1u + sizeof(data3) + 1u);
    if (!c4.empty()) {
