...
TEST(AES, ECBVector)
{
    ...
    EXPECT_TRUE(aes_ecb_encrypt(out, key, in));
...
TEST(AES, CBCVector)
{
    ...
    EXPECT_TRUE(aes_cbc_crypt(out, key, iv, in, sizeof(in), true));
    EXPECT_TRUE(std::memcmp(out, expected, 16) == 0);

    // Decrypt back
    EXPECT_TRUE(aes_cbc_crypt(back, key, iv, out, sizeof(out), false));
...
TEST(AES, CMACVectors)
{
    ...
    EXPECT_TRUE(cmac_aes_128(out, key, msg2, sizeof(msg2)));
    ...
    hex_to_bytes(
        ...
        msg3, 64);
    EXPECT_TRUE(cmac_aes_128(out, key, msg3, sizeof(msg3)));
